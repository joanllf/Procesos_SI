...
void dibujaZonaLogo(){
  pushStyle();
    fill(200,50,100);
    rect(width-marginH-logoWidth, marginV, logoWidth, logoHeight);
    fill(0);
    text("LOGO", width-marginH-logoWidth + logoWidth/2, marginV + logoHeight/2);
  popStyle();
}

void dibujaZonaPista(){
  pushStyle();
    fill(200, 50, 250);
    rect(marginH, marginV, pistaWidth, pistaHeight);
  popStyle();
}

void dibujaZonaMarcador(){
  pushStyle();
    fill(55, 170, 0);    
    rect(2 * marginH + pistaWidth, 2 * marginV + logoHeight, marcadorWidth, marcadorHeight);
  popStyle();
}
